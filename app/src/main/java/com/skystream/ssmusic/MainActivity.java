...
public class MainActivity extends AppCompatActivity {
    ...
    static String videoDisplayScript(boolean showThumbnailByDefault, String showThumbnailLabel,
        ...
        return "(function(){"
                ...
                + "var selectors=['ytmusic-player-page tp-yt-paper-icon-button[aria-label=\"Minimize player\"]',"
                ...
                + "'ytmusic-player-page #minimize-player-button',"
                + "'ytmusic-player-page [icon=\"yt-icons:keyboard-arrow-down\"]',"
                + "'ytmusic-player-page [icon=\"yt-icons:expand-more\"]'];"
                + "for(var i=0;i<selectors.length;i++){if(clickFirstVisible(selectors[i])){return true;}}"
                + "var page=document.querySelector('ytmusic-player-page');"
                + "var buttons=page?page.querySelectorAll('tp-yt-paper-icon-button,button,[role=\"button\"]'):[];"
                + "for(var j=0;j<buttons.length;j++){"
                + "var label=((buttons[j].getAttribute('aria-label')||'')+' '+(buttons[j].getAttribute('title')||'')).toLowerCase();"
                + "if((label.indexOf('minimize')!==-1||label.indexOf('down arrow')!==-1)&&visible(buttons[j])){buttons[j].click();return true;}"
                + "}"
                + "return clickFirstVisible('ytmusic-player-page ytmusic-player-page-header tp-yt-paper-icon-button:first-of-type');"
                + "}"
                ...
                + "if(dy>DRAG_SLOP_PX&&Math.abs(dy)>Math.abs(dx)*1.2){dragging=true;if(event.cancelable){event.preventDefault();}}"
                + "}"
                + "function end(event){"
                + "if(!tracking){return;}"
                + "var p=point(event);var dx=p.x-startX;var dy=p.y-startY;"
                + "var shouldMinimize=dragging&&dy>DRAG_MINIMIZE_PX&&Math.abs(dy)>Math.abs(dx)*1.2;"
                + "reset();"
