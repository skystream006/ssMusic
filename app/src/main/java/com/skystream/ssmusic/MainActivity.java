...
public class MainActivity extends AppCompatActivity {
    ...
    static String videoDisplayScript(boolean showThumbnailByDefault, String showThumbnailLabel,
        ...
        return "(function(){"
                ...
                + "ytmusic-player-page tp-yt-paper-icon-button[icon=\"yt-icons:keyboard-arrow-down\"],"
                + "ytmusic-player-page tp-yt-paper-icon-button[icon=\"yt-icons:expand-more\"]';"
                + "var PREVIOUS_BUTTON_SELECTOR='ytmusic-player-bar .previous-button,tp-yt-paper-icon-button.previous-button';"
                + "var NEXT_BUTTON_SELECTOR='ytmusic-player-bar .next-button,tp-yt-paper-icon-button.next-button';"
                ...
                + "return clickFirstVisible(MINIMIZE_BUTTON_SELECTOR);"
                + "}"
                + "function previousSong(){return clickFirstVisible(PREVIOUS_BUTTON_SELECTOR);}"
                + "function nextSong(){return clickFirstVisible(NEXT_BUTTON_SELECTOR);}"
                + "function installVideoDrag(node){"
                + "if(node.__ssmusicMinimizeDragInstalled){return;}"
                + "node.__ssmusicMinimizeDragInstalled=true;"
                + "var startX=0,startY=0,tracking=false,dragging=false;"
                + "function point(event){var touch=(event.changedTouches&&event.changedTouches[0])||(event.touches&&event.touches[0])||event;return {x:touch.clientX,y:touch.clientY};}"
                + "function reset(){tracking=false;dragging=false;}"
                + "function start(event){var p=point(event);startX=p.x;startY=p.y;tracking=true;dragging=false;}"
                + "function isVerticalDrag(dx,dy){return Math.abs(dy)>Math.abs(dx)*DRAG_AXIS_RATIO;}"
                + "function isHorizontalDrag(dx,dy){return Math.abs(dx)>Math.abs(dy)*DRAG_AXIS_RATIO;}"
                + "function move(event){"
                + "if(!tracking){return;}"
                + "var p=point(event);var dx=p.x-startX;var dy=p.y-startY;"
                + "var vertical=Math.abs(dy)>DRAG_SLOP_PX&&isVerticalDrag(dx,dy);"
                + "var horizontal=Math.abs(dx)>DRAG_SLOP_PX&&isHorizontalDrag(dx,dy);"
                + "if(vertical||horizontal){dragging=true;if(event.cancelable){event.preventDefault();}}"
                + "}"
                + "function end(event){"
                + "if(!tracking){return;}"
                + "var p=point(event);var dx=p.x-startX;var dy=p.y-startY;"
                + "var shouldMinimize=dragging&&dy>DRAG_MINIMIZE_PX&&isVerticalDrag(dx,dy);"
                + "var shouldNavigate=dragging&&Math.abs(dx)>DRAG_NAVIGATE_PX&&isHorizontalDrag(dx,dy);"
                + "reset();"
