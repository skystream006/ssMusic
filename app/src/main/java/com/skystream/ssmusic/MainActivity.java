...
public class MainActivity extends AppCompatActivity {
    ...
    static String videoDisplayScript(boolean showThumbnailByDefault, String showThumbnailLabel,
        ...
        return "(function(){"
                ...
                + "if(dy>DRAG_SLOP_PX&&Math.abs(dy)>Math.abs(dx)*DRAG_VERTICAL_RATIO){dragging=true;if(event.cancelable){event.preventDefault();}}"
                + "}"
                + "function end(event){"
                + "if(!tracking){return;}"
                + "var p=point(event);var dx=p.x-startX;var dy=p.y-startY;"
                + "var shouldMinimize=dragging&&dy>DRAG_MINIMIZE_PX&&Math.abs(dy)>Math.abs(dx)*DRAG_VERTICAL_RATIO;"
                + "reset();"
