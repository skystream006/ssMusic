...
public class MainActivity extends AppCompatActivity {
    ...
    static String videoDisplayScript(boolean showThumbnailByDefault, String showThumbnailLabel,
        ...
        return "(function(){"
                ...
                + "try{"
                + "if(window.ssmusicPlayback&&window.ssmusicPlayback.logDiagnostic){"
                + "var command=action==='down'?'minimize':action==='left'?'previous':'next';"
                ...
                + "if(!target.closest||target.closest('button,a,input,select,textarea,[role=\"button\"],[role=\"slider\"],"
                + "tp-yt-paper-icon-button,yt-icon-button,.ytp-chrome-bottom,#'+ROOT_ID)){return;}"
                + "var node=video();var page=mediaPage(node);"
                ...
                + "if(action&&swipeControl(action)){swipe.action=action;}"
                + "}"
                ...
                + "},{capture:true,passive:false});"
                + "document.addEventListener('touchcancel',function(){swipe=null;},{capture:true,passive:true});"
                + "}"
