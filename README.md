...
state, media notification commands, and uncaught exceptions — together with a stack trace for
every entry. Entries go to logcat and to a private log file (`files/logs/ssmusic.log`) that is
rotated once it reaches 512 KB. Use **Share log** to send the file to another app and
**Clear log** to delete it. Because the log records visited YouTube Music URLs and track
metadata, only share it with people you trust.
